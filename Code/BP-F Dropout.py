...
# Function to build and train the model with Dropout
def build_and_train_model(layers_config, learning_rate, epochs, activation_fn, dropout_rate=0.3):
    model = models.Sequential()
    model.add(layers.Input(shape=(X_train.shape[1],)))  # Input layer
    for units in layers_config:  # Hidden layers
        model.add(layers.Dense(units, activation=activation_fn))
        model.add(layers.Dropout(dropout_rate))  # Adding Dropout
...
hyperparameter_combinations = [
    {"layers": [128, 64], "epochs": 50, "learning_rate": 0.01, "activation_fn": 'relu', "dropout_rate": 0.3},
    {"layers": [128, 64], "epochs": 50, "learning_rate": 0.01, "activation_fn": 'relu', "dropout_rate": 0.5},
    {"layers": [128, 64], "epochs": 50, "learning_rate": 0.01, "activation_fn": 'relu', "dropout_rate": 0.2}
]
...
for config in hyperparameter_combinations:
    ...
    mape, mae, mse, history, y_pred = build_and_train_model(
        config['layers'], config['learning_rate'], config['epochs'], config['activation_fn'], config['dropout_rate']
    )
    ...
    plt.title(f"Training and Validation Loss for {config['layers']} Layers with Dropout Rate {config['dropout_rate']}")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    plt.show()
    ...
    plt.title(f"Predicted vs Actual for {config['layers']} Layers with Dropout Rate {config['dropout_rate']}")
    plt.legend()
    plt.show()
...
results_path = r'C:\Users\Arif Bhuiyan\Desktop\A1 Final\hyperparameter_results_BP-F_dropout.csv'
